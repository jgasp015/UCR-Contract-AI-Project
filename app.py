...
def scrape_stable_bids(url):
    """Reliable scraper that won't crash the server."""
    it_keywords = ["software", "hardware", "network", "cabling", "saas", "cloud", "it ", "technology"]
    junk_words = ["javascript", "detached", "loading", "refresh", "login"]
    ...
    try:
        ...
        for row in soup.find_all(['tr', 'div', 'li', 'span']):
            ...
            if any(k in text.lower() for k in it_keywords) and not any(j in text.lower() for j in junk_words):
                if len(text) > 45:
                    clean_name = text[:100].upper()
...
if st.session_state.active_bid_text is None:
    with input_container.container():
        if input_mode == "Upload PDF":
            ...
            if url_input:
                with st.spinner("Scanning for technology bids..."):
                    ...
                    if bids:
                        ...
                        st.warning("No bids found. Note: Some portals require JavaScript and must be analyzed via Upload PDF.")
