...
def deep_query(full_text, specific_prompt, max_tokens=None):
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": "You are a concise procurement expert. Give direct answers."},
...
if st.session_state.active_bid_text:
    if not st.session_state.summary_ans:
        with st.status("🚀 Running Deep Scan...") as status:
            ...
            # --- STRICT DATE LOGIC ---
            date_found = deep_query(doc, "Extract ONLY the bid due date (e.g., April 24, 2026).", max_tokens=15)
            is_awarded = deep_query(doc, "Has a specific vendor already been selected? Answer YES or NO.", max_tokens=5)
            
            if "YES" in is_awarded.upper():
                st.session_state.status_flag = "AWARDED"
            elif "APRIL" in date_found and ("24" in date_found or "25" in date_found or "30" in date_found):
                st.session_state.status_flag = "OPEN"
            else:
                st.session_state.status_flag = deep_query(doc, "Today is April 19, 2026. Is this bid OPEN or CLOSED? One word.", max_tokens=5)
            ...
            st.session_state.submission_ans = f"**Close Date Detected: {date_found}**\n\n" + deep_query(doc, "Identify submission steps.")
    ...
    if "OPEN" in clean_status or "ACTIVE" in clean_status:
        st.success(f"✅ STATUS: OPEN (Due: {time.strftime('%B %d, %Y')})")
